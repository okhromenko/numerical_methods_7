...
def table_output(cons, b, z, text):
    table = PrettyTable()
    table.field_names = ["x1", "x2", "x3", "x4", "x5", "x6", "x7", "x8", "x9", "x10", "x11", "x12", "x13", "x14", "x15", "x16", "b"]
    print(text)
    for i in range(len(cons)):
        table.add_row([cons[i][0], cons[i][1], cons[i][2], cons[i][3], cons[i][4], cons[i][5], cons[i][6], cons[i][7], cons[i][8],
                       cons[i][9], cons[i][10], cons[i][11], cons[i][12], cons[i][13], cons[i][14], cons[i][15], b[i]])
    table.add_row([z[0], z[1], z[2], z[3], z[4], z[5], z[6], z[7], z[8], z[9], z[10], z[11], z[12], z[13], z[14], z[15], b[len(b) - 1]])
    print(table)
...
def simplex(cons, z, b):
    ...
    for i in range(6, 14):
        basis[i] = True
    letters = ['x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7', 'x8', 'x9', 'x10', 'x11', 'x12', 'x13', 'x14', 'x15', 'x16']
    ...
    print('\nОтвет: ', end="")
    print('z(max) = z({},{},{},{},{},{}) = {}'.format(basis[0], basis[1], basis[2], basis[3], basis[4], basis[5], basis[11], basis[12], b[len(b) - 1]))
    print('\n')